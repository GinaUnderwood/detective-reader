...
const SCREEN_ELEVEN_NARRATION="Let's review. Click on the words that match this case pattern.";
const SCREEN_ELEVEN_PATTERN_FLASH_MS=2000;
const SCREEN_TWELVE_NARRATION='Now, you are ready for the Challenge words. Read these words. Some of the words may be nonsense words.';
...
function stopNarration(){
  ...
  if(screenElevenRoot?.isConnected){
    ...
    pattern?.classList.remove('active','revealed');
    pattern?.setAttribute('aria-hidden','true');
  }
  if(screenTwelveRoot?.isConnected){
    screenTwelveRoot.querySelectorAll('[data-screen-twelve-word].active').forEach(word=>word.classList.remove('active'));
    screenTwelveRoot.querySelector('[data-screen-twelve-confetti]')?.classList.remove('active');
    screenTwelveRoot.classList.remove('celebrating');
  }
  if(screenThirteenRoot?.isConnected){
...
function screenTwelveWords(){return ['ch','tr','sl','gr','dr'].map(onset=>onset+ending()).filter(word=>word.toLowerCase()!=='chit')}
function screenTwelveWordCards(words){return `<div class="wordRow challengeWordRow" aria-label="Challenge words">${words.map((w,i)=>`<div class="word screenFourWord" data-screen-twelve-word="${i}" data-spoken="${w}">${w}</div>`).join('')}</div>`}
...
function renderLesson(){const l=lesson(),s=state.screen,tag=(n)=>`<span class="screenTag">SCREEN ${n} OF ${screenNames.length} · ${screenNames[n-1]}</span>`;
  ...
  if(s===12){const big=screenTwelveWords();return lessonFrame(`${screenTwelveConfetti()}${tag(12)}<h1>Challenge clues</h1><p>You know ${l.pattern}. Try these challenge words.</p>${screenTwelveWordCards(big)}<p class="screenFourQuestion" data-screen-twelve-status aria-live="polite">${SCREEN_TWELVE_NARRATION}</p><div class="footerActions"><button class="secondary hearWordsButton" data-screen-twelve-control data-screen-twelve-read aria-label="Hear all the challenge words"><span class="humanEarIcon" aria-hidden="true">👂</span><span>Hear the Words</span></button><button class="primary" data-screen-twelve-control data-screen-twelve-complete>Continue →</button></div>`)}
...
function startScreenElevenSequence(){
  stopNarration();
  const run=++screenElevenRun,lessonIndex=state.lesson,root=document.querySelector('.patternHunt');
  const pattern=root?.querySelector('[data-screen-eleven-pattern]'),choices=[...root?.querySelectorAll('[data-hunt]')||[]];
  if(state.view!=='lesson'||state.screen!==11||!root||!pattern||!choices.length)return;
  screenElevenRoot=root;
  state.huntReady=false;
  choices.forEach(choice=>choice.disabled=true);
  pattern.classList.remove('active','revealed');
  pattern.setAttribute('aria-hidden','true');
  const isCurrent=()=>run===screenElevenRun&&state.view==='lesson'&&state.screen===11&&state.lesson===lessonIndex&&root.isConnected;
  const flashPattern=()=>{
    if(!isCurrent())return;
    pattern.classList.add('active');
    pattern.setAttribute('aria-hidden','false');
    screenElevenTimer=setTimeout(()=>{
      if(!isCurrent())return;
      pattern.classList.remove('active');
      pattern.classList.add('revealed');
      state.huntReady=true;
      choices.forEach(choice=>choice.disabled=false);
    },SCREEN_ELEVEN_PATTERN_FLASH_MS);
  };
...
function startScreenTwelveSequence(){
  stopNarration();
  const root=document.querySelector('.challengeWords');
  const status=root?.querySelector('[data-screen-twelve-status]'),continueButton=root?.querySelector('[data-screen-twelve-complete]'),hearButton=root?.querySelector('[data-screen-twelve-read]'),cards=[...root?.querySelectorAll('[data-screen-twelve-word]')||[]],confetti=root?.querySelector('[data-screen-twelve-confetti]');
  if(state.view!=='lesson'||state.screen!==12||!root||!status||!continueButton||!hearButton||!confetti||cards.length!==screenTwelveWords().length)return;
  ...
  speak(SCREEN_TWELVE_NARRATION,.78);
}

function readScreenTwelveWords(){
  stopNarration();
  const run=++screenTwelveRun,lessonIndex=state.lesson,root=document.querySelector('.challengeWords');
  const words=[...root?.querySelectorAll('[data-screen-twelve-word]')||[]],status=root?.querySelector('[data-screen-twelve-status]'),hearButton=root?.querySelector('[data-screen-twelve-read]');
  if(state.view!=='lesson'||state.screen!==12||!root||!words.length||!status||!hearButton)return;
  screenTwelveRoot=root;
  const clearHighlights=()=>words.forEach(word=>word.classList.remove('active'));
  const isCurrent=()=>run===screenTwelveRun&&state.view==='lesson'&&state.screen===12&&state.lesson===lessonIndex&&root.isConnected;
  const fail=()=>{
    if(!isCurrent())return;
    clearHighlights();
    status.textContent='Narration is unavailable. Select Hear the Words to try again.';
  ...
  const readWord=(index)=>{
    ...
    if(index>=words.length){
      clearHighlights();
      status.textContent=`You heard all ${words.length} challenge words.`;
      return;
    }
    const word=words[index];
    speak(word.dataset.spoken,.68,{
      onStart:(result={})=>{
        if(!isCurrent()||result.unavailable)return;
        clearHighlights();
        word.classList.add('active');
      ...
      onComplete:(result={})=>{
        ...
        word.classList.remove('active');
        if(result.error||result.unavailable){fail();return}
        screenTwelveTimer=setTimeout(()=>{if(isCurrent())readWord(index+1)},300);
      }
    });
  };
  clearHighlights();
  status.textContent=`Reading all ${words.length} challenge words aloud. Follow each highlighted word.`;
  readWord(0);
}
...
function completeScreenTwelve(){
  stopNarration();
  const run=++screenTwelveRun,lessonIndex=state.lesson,root=document.querySelector('.challengeWords');
  const status=root?.querySelector('[data-screen-twelve-status]'),continueButton=root?.querySelector('[data-screen-twelve-complete]'),controls=[...root?.querySelectorAll('[data-screen-twelve-control]')||[]],hearButton=root?.querySelector('[data-screen-twelve-read]'),cards=[...root?.querySelectorAll('[data-screen-twelve-word]')||[]],confetti=root?.querySelector('[data-screen-twelve-confetti]');
  if(state.view!=='lesson'||state.screen!==12||!root||!status||!continueButton||!hearButton||!confetti||cards.length!==screenTwelveWords().length||controls.length!==2||continueButton.disabled)return;
  screenTwelveRoot=root;
  ...
  root.classList.add('celebrating');
  confetti.classList.add('active');
  toast('Challenge words complete! ⭐');
  screenTwelveTimer=setTimeout(()=>{
    if(run!==screenTwelveRun||state.view!=='lesson'||state.screen!==12||state.lesson!==lessonIndex||!root.isConnected)return;
    confetti.classList.remove('active');
    root.classList.remove('celebrating');
    goToLessonScreen(13);
  },SCREEN_TWELVE_CONFETTI_MS);
}

function startScreenThirteenSequence(){
  stopNarration();
  const run=++screenThirteenRun,lessonIndex=state.lesson,root=document.querySelector('.masteryCheck');
  const status=root?.querySelector('[data-screen-thirteen-status]'),button=root?.querySelector('[data-screen-thirteen-action]'),confetti=root?.querySelector('[data-screen-thirteen-confetti]');
  if(state.view!=='lesson'||state.screen!==13||!root||!status||!button||!confetti)return;
  screenThirteenRoot=root;
  confetti.classList.remove('active');
  root.classList.remove('celebrating');
  button.disabled=true;
  status.textContent=SCREEN_THIRTEEN_NARRATION;
  const isCurrent=()=>run===screenThirteenRun&&state.view==='lesson'&&state.screen===13&&state.lesson===lessonIndex&&root.isConnected&&status.isConnected;
  speak(SCREEN_THIRTEEN_NARRATION,.78,{onComplete:(result={})=>{
    if(!isCurrent())return;
    button.disabled=false;
    if(result.error||result.unavailable){
      status.textContent='Narration is unavailable. Say each word and click below for the next word.';
      toast('Narration is unavailable. Select Play to try again.');
    }
  }});
}

function replayScreenThirteen(){
  if(document.querySelector('.masteryCheck.celebrating'))return;
  startScreenThirteenSequence();
}

function completeScreenThirteen(){
  const lessonIndex=state.lesson,root=document.querySelector('.masteryCheck');
  const status=root?.querySelector('[data-screen-thirteen-status]'),button=root?.querySelector('[data-screen-thirteen-action]'),confetti=root?.querySelector('[data-screen-thirteen-confetti]');
  if(state.view!=='lesson'||state.screen!==13||state.mastery!==9||!root||!status||!button||!confetti||button.disabled)return;
  stopNarration();
  const run=++screenThirteenRun;
  screenThirteenRoot=root;
  button.disabled=true;
  status.textContent='Mastery check complete!';
  root.classList.add('celebrating');
  confetti.classList.add('active');
  toast('Mastery check complete! ⭐');
  screenThirteenTimer=setTimeout(()=>{
    if(run!==screenThirteenRun||state.view!=='lesson'||state.screen!==13||state.lesson!==lessonIndex||state.mastery!==9||!root.isConnected)return;
    confetti.classList.remove('active');
    root.classList.remove('celebrating');
    state.mastery=0;
    goToLessonScreen(14);
  },SCREEN_THIRTEEN_CONFETTI_MS);
}

function advanceScreenThirteen(){
  if(state.view!=='lesson'||state.screen!==13)return;
  if(state.mastery>=9){completeScreenThirteen();return}
  stopNarration();
  state.mastery++;
  render();
...
function goToLessonScreen(next){
  ...
  if((state.screen===11&&target!==11)||(target===11&&state.screen!==11))resetScreenEleven();
  if((state.screen===13&&target!==13)||(target===13&&state.screen!==13))state.mastery=0;
  state.screen=target;
  render();
  scrollPageToTop();
...
function bind(){
  ...
  document.querySelectorAll('[data-screen-twelve-read]').forEach(x=>x.onclick=readScreenTwelveWords);
...
function resetLesson(){Object.assign(state,{readIndex:0,buildWords:[],buildIndex:null,hunt:[],huntReady:false,mastery:0})}
render();
